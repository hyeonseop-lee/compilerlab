...
public:
	...
	virtual int toInst(Scope *scope, function<void(int, string, string)> warning, function<void(int, string, string)> error);
};
...
public:
	...
	int lalloc();
	void free(int reg);
	void addVar(Symbol *_symbol, int loc);
	bool isLocal(Symbol *_symbol);
	int getVar(Symbol *_symbol);
};
